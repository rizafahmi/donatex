defmodule NotableWeb.MayarWebhookControllerTest do
  ...
  defmodule TestStub do
    @behaviour Notable.Mayar.Client.Impl
  ...
  setup do
    ...
    Application.put_env(:notable, :mayar_client_impl, TestStub)

    on_exit(fn ->
      if original_impl do
        Application.put_env(:notable, :mayar_client_impl, original_impl)
      else
  ...
  test "returns 500 and does not broadcast when transaction ID update fails", %{
    ...
    Application.put_env(:notable, :mayar_client_impl, LookupStub)
  ...
  test "programmer errors in Mayar lookup propagate (not swallowed into amount fallback)",
    ...
    Application.put_env(:notable, :mayar_client_impl, RaisingStub)
