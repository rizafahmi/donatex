defmodule NotableWeb.AdminAnalyticsTest do
  ...
  setup do
    ...
    Repo.delete_all(Notable.Analytics.PageView)
    Repo.delete_all(Notable.Donations.Donation)
    :ok
  end
  ...
  test "admin panel renders conversion funnel and updates in real-time", %{conn: conn} do
    ...
    Phoenix.PubSub.broadcast(
      Notable.PubSub,
      "donations:paid",
      {:donation_paid, NotableWeb.DonationPresenter.payload(paid_tip)}
    )
  ...
  test "admin panel handles capping conversion rates when views < feedback + tips", %{conn: conn} do
    ...
    %Notable.Analytics.PageView{path: "/"} |> Repo.insert!()
