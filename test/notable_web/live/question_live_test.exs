defmodule NotableWeb.QuestionLiveTest do
  ...
  describe "submission" do
    ...
    test "missing visitor session fails safely without inventing an identity", %{conn: conn} do
      {:ok, view, html} = live_isolated(conn, NotableWeb.QuestionLive, session: %{})
    ...
    test "create constraint errors re-render on question form inputs", %{conn: conn} do
      original = Application.get_env(:notable, :questions)

      on_exit(fn ->
        if original do
          Application.put_env(:notable, :questions, original)
        else
          Application.delete_env(:notable, :questions)
        end
      end)

      Application.put_env(:notable, :questions, PersistFailingQuestions)
  ...
  describe "voting" do
    ...
    test "already_voted race reloads board without crashing QuestionLive", %{conn: conn} do
      ...
      Application.put_env(:notable, :questions, AlreadyVotedQuestions)
      ...
      assert [_vote] = Repo.all(Notable.Questions.QuestionVote)
  ...
  describe "midnight rollover" do
    test "rolls today into a collapsed historical group at the WIB boundary", %{conn: conn} do
      ...
      {:ok, view, _html} =
        live_isolated(conn, NotableWeb.QuestionLive, session: %{"current_now" => before_midnight})
