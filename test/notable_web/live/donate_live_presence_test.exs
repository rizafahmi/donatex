defmodule NotableWeb.DonateLivePresenceTest do
  use NotableWeb.ConnCase, async: false

  import ExUnit.CaptureLog
  import Phoenix.LiveViewTest

  alias NotableWeb.DonateLive
  ...
  setup do
    original = Application.get_env(:notable, :visitor_presence)
  ...
  test "hides low counts, then updates all visitors at the threshold", %{conn: conn} do
    topic = unique_topic()
    :ok = Phoenix.PubSub.subscribe(Notable.PubSub, topic)
  ...
  defp with_visitor_presence(mod, fun) do
    original = Application.get_env(:notable, :visitor_presence)
    Application.put_env(:notable, :visitor_presence, mod)
  ...
  defp restore_visitor_presence(nil), do: Application.delete_env(:notable, :visitor_presence)
  defp restore_visitor_presence(mod), do: Application.put_env(:notable, :visitor_presence, mod)
