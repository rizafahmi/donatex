defmodule DonatexWeb.QrOverlayLiveTest do
  ...
  describe "free Note floats" do
    test "shows an emoji-only float for an accepted free Note", %{conn: conn} do
      {:ok, feedback} =
        Donatex.Donations.create_feedback(%{
    ...
    test "animates a free Note float for three to four seconds", %{conn: conn} do
      {:ok, feedback} =
        Donatex.Donations.create_feedback(%{donor_name: "Timed", reaction: "great", message: nil})
    ...
    test "does not float pending tip creations on donations:created", %{conn: conn} do
      {:ok, pending} =
        Donatex.Donations.create_pending_donation(%{
    ...
    test "shows multiple free Note floats simultaneously", %{conn: conn} do
      {:ok, first} =
        Donatex.Donations.create_feedback(%{donor_name: "One", reaction: "ok", message: nil})

      {:ok, second} =
        Donatex.Donations.create_feedback(%{donor_name: "Two", reaction: "bad", message: nil})
    ...
    test "dismisses a free Note float after the float timer", %{conn: conn} do
      {:ok, feedback} =
        Donatex.Donations.create_feedback(%{donor_name: "Gone", reaction: "great", message: nil})
