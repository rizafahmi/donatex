defmodule Notable.SqliteBench do
  @moduledoc """
  Concurrent SQLite read/write A/B harness for throwaway databases.

  Compares Notable.Repo production concurrency knobs (WAL + busy_timeout 5000 +
  IMMEDIATE) against a deliberately worse baseline (DELETE journal / busy_timeout
  0 / DEFERRED). Invoked via `mix notable.sqlite_bench`; not part of `mix ci`.
  """
  ...
  @doc """
  Production-intent knobs matching `Notable.Repo` in config/config.exs.
  """
  ...
  def run_ab!(dir, opts \\ %{}) when is_binary(dir) do
    ...
    optimized =
      run_variant!(
        Notable.SqliteBench.OptimizedRepo,
        :optimized,
        @optimized_label,
    ...
    baseline =
      run_variant!(
        Notable.SqliteBench.BaselineRepo,
        :baseline,
        @baseline_label,
  ...
  def format_report(%{optimized: optimized, baseline: baseline, opts: opts}) do
    [
      "Notable SQLite concurrent A/B bench",
...
defmodule Notable.SqliteBench.OptimizedRepo do
  @moduledoc false
  use Ecto.Repo, otp_app: :notable, adapter: Ecto.Adapters.SQLite3
end

defmodule Notable.SqliteBench.BaselineRepo do
  @moduledoc false
  use Ecto.Repo, otp_app: :notable, adapter: Ecto.Adapters.SQLite3
end
