...
- **R3**: Persist page views to a SQLite table named `page_views`.
  - *Trace*: Satisfied by migration and `Donatex.Analytics.PageView` schema.
...
- **R6**: Show Total Views, Feedback count + %, Tip count + %.
  - *Trace*: Calculations performed in `Donatex.Analytics.get_funnel_stats/0`.
...
- Schema creation pattern: [Donation Schema](file:///Users/riza/code/donatex/lib/donatex/donations/donation.ex) (uses `:binary_id` primary key, `timestamps(type: :utc_datetime)`).
...
- LiveView PubSub subscription and real-time updates: [AdminLive mount and handle_info](file:///Users/riza/code/donatex/lib/donatex_web/live/admin_live.ex).
...
- **Question**: Where do we put the database queries?
  - *Resolution*: A new context `Donatex.Analytics` will keep these queries isolated from the `Donatex.Donations` context.
...
- [x] **Unit 1: Database Migration and Ecto Schema**
  ...
  - **Files**:
    ...
    - Create: `lib/donatex/analytics/page_view.ex`
  - **Approach**:
...
- [x] **Unit 2: Analytics Context Module**
  ...
  - **Dependencies**: Unit 1
  - **Files**:
    - Create: `lib/donatex/analytics.ex`
  - **Approach**:
...
- [x] **Unit 3: Context & Integration Tests**
  ...
  - **Dependencies**: Unit 2
  - **Files**:
    - Create: `test/donatex/analytics_test.exs`
  - **Approach**:
  ...
  - **Verification**: `mix test test/donatex/analytics_test.exs` passes successfully.
...
- [x] **Unit 4: LiveView Integration for Page Views**
  ...
  - **Dependencies**: Unit 2
  - **Files**:
    - Modify: `lib/donatex_web/live/donate_live.ex`
  - **Approach**:
    - Inside `mount/3`, check if `connected?(socket)` is true. If so, call `Donatex.Analytics.track_page_view("/")`.
  - **Test scenarios**:
...
- [x] **Unit 5: Admin Panel Funnel UI and Real-Time updates**
  ...
  - **Files**:
    - Modify: `lib/donatex_web/live/admin_live.ex`
    - Test: `test/donatex_web/features/admin_analytics_test.exs`
  - **Approach**:
    - In `mount/3`, subscribe to `"analytics:page_view"`.
    - Assign `@funnel_stats` in socket assigns using `Donatex.Analytics.get_funnel_stats/0`.
  ...
  - **Verification**: Run tests in `test/donatex_web/features/admin_analytics_test.exs` and ensure they pass.
