...
## In Progress
- [#26](https://github.com/rizafahmi/donatex/issues/26) Harden amount-fallback payment correlation — branch `fix/26-harden-amount-fallback-correlation`: atomic amount (+ optional donor_name) claim with fail-closed ambiguous matches; edge-case tests green.
- [#25](https://github.com/rizafahmi/donatex/issues/25) Make paid webhook transitions concurrency-safe — merged on main (`bc11ce3`).
- [#24](https://github.com/rizafahmi/donatex/issues/24) Questions WIB/today empty listing — fixed with #25 by making questions tests calendar-relative to `today_wib()`.
...
## Next Steps
1. Finish #26 (codex-loop + no-mistakes / PR), then webhook ops hardening (#31).
