...
- Created a new [SEO Plug](file:///Users/riza/code/donatex/lib/donatex_web/plugs/seo.ex) that runs on the browser pipeline to dynamically fetch the configured application base URL and populate connection assigns (`:meta_description`, `:meta_robots`, `:canonical_url`) based on the request path.
- Registered `DonatexWeb.Plugs.SEO` inside the `:browser` pipeline in [router.ex](file:///Users/riza/code/donatex/lib/donatex_web/router.ex).
- Updated [root.html.heex](file:///Users/riza/code/donatex/lib/donatex_web/components/layouts/root.html.heex) to read these assigns and render description, robots, canonical links, Open Graph (og:type, og:title, og:description, og:url, og:site_name), and Twitter (twitter:card, twitter:title, twitter:description) tags.

### Dynamic JSON-LD Structured Data
- Injected Organization and FAQPage schemas as JSON-LD `<script type="application/ld+json">` blocks at the bottom of the landing page template in [donate_live.ex](file:///Users/riza/code/donatex/lib/donatex_web/live/donate_live.ex). This includes structured details about the "Notable" brand and a fully descriptive Indonesian FAQ structure mapping landing page functionality.
...
- Added a link to Riza's personal website directly inside the landing page description copy on [donate_live.ex](file:///Users/riza/code/donatex/lib/donatex_web/live/donate_live.ex).
...
- Added a permanent `Tentang Riza` header navigation link to [layouts.ex](file:///Users/riza/code/donatex/lib/donatex_web/components/layouts.ex) so users on administrative and public pages can easily find the streamer's background.
- Ensured all outgoing links use `target="_blank"` and `rel="noopener"` safeties.

### Static Asset Routing and Optimization
- Modified `static_paths/0` in [donatex_web.ex](file:///Users/riza/code/donatex/lib/donatex_web.ex) to ensure new static assets `sitemap.xml` and `llms.txt` are served directly by the web server.
...
- Added `strict-transport-security` (`max-age=31536000; includeSubDomains`) to [security_headers.ex](file:///Users/riza/code/donatex/lib/donatex_web/security_headers.ex) to pass Lighthouse technical trust signal gates.
- Updated the `/donate` redirection handler in [page_controller.ex](file:///Users/riza/code/donatex/lib/donatex_web/controllers/page_controller.ex) to send a `301 Moved Permanently` status instead of a temporary `302 Found` status.

### Test Coverage and Assertions
- Created [static_seo_test.exs](file:///Users/riza/code/donatex/test/donatex_web/controllers/static_seo_test.exs) to test successful delivery of static assets, HTTP header safety, and 301 redirect behavior.
- Added test cases in [donate_live_test.exs](file:///Users/riza/code/donatex/test/donatex_web/live/donate_live_test.exs) and [overlay_live_test.exs](file:///Users/riza/code/donatex/test/donatex_web/live/overlay_live_test.exs) to verify that metadata descriptions, robots index/noindex directives, canonical link tags, OG/Twitter tags, and JSON-LD schemas render properly.
- Updated [access_control_test.exs](file:///Users/riza/code/donatex/test/donatex_web/features/access_control_test.exs) to verify `/admin` returns `noindex` and the correct canonical link header.
- Updated [page_controller_test.exs](file:///Users/riza/code/donatex/test/donatex_web/controllers/page_controller_test.exs) to match the new 301 status assertions.
- Fixed mismatched checkboxes in [donor_qr_flow_test.exs](file:///Users/riza/code/donatex/test/donatex_web/features/donor_qr_flow_test.exs) to resolve failures from previous local UI changes.
