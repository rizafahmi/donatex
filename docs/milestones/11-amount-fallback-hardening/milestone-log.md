...
### Tests
- `test/notable/donations_test.exs` — 6 new `claim_pending_by_amount/3` tests: unique claim, ambiguous fail-closed, donor_name disambiguation, orphan, same-name ambiguous, already-paid no-claim
- `test/notable_web/controllers/mayar_webhook_controller_test.exs` — 3 new integration tests: ambiguous fail-closed (no paid mark, no broadcast), donor_name disambiguation via webhook, orphan payment
