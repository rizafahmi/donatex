...
- Optional concurrent A/B bench (not in `mix ci`): `mix donatex.sqlite_bench` runs the same majority-read / minority-write mix on throwaway DBs for optimized knobs vs a worse baseline (`DELETE` journal, `busy_timeout: 0`, deferred txs) and prints throughput plus lock-error counts. Flags: `--workers`, `--ops`, `--write-pct`, `--keys`, `--seed`, `--dir`.
