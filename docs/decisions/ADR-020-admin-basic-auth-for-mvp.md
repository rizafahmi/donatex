...
Notable is a single-streamer system with a small administrative surface:
...
- the router applies a dedicated `:admin` pipeline via `NotableWeb.Plugs.AdminBasicAuth` (`Plug.BasicAuth`), which stamps `admin_authenticated` into the session on success
- admin live routes are wrapped in `live_session :admin` with `NotableWeb.LiveAdminAuth` `on_mount`, because LiveView websocket mount/reconnect bypasses the plug pipeline
...
- Rejected because Notable is explicitly single-user and does not need accounts for the MVP
